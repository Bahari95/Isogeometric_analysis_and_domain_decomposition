...
#define MOD_R4NI7IX97UIY_WRAPPER

#include "mod_r4ni7ix97uiy_wrapper.h"
#include <stdlib.h>
#include <stdint.h>
#include "ndarrays.h"
...
static PyObject* bind_c_assemble_matrix_un_ex01_wrapper(PyObject* self, PyObject* args, PyObject* kwargs)
{
    ...
    double domain_nb;
    double S_DDM;
    t_ndarray ovlp_value = {.shape = NULL};
    void* bound_ovlp_value;
    int64_t bound_ovlp_value_shape_1;
    int64_t bound_ovlp_value_shape_2;
    int64_t bound_ovlp_value_stride_1;
    int64_t bound_ovlp_value_stride_2;
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "OOOOOOOOOOOOOOOOOO", kwlist, &ne1_obj, &ne2_obj, &p1_obj, &p2_obj, &spans_1_obj, &spans_2_obj, &basis_1_obj, &basis_2_obj, &weights_1_obj, &weights_2_obj, &points_1_obj, &points_2_obj, &domain_nb_obj, &S_DDM_obj, &ovlp_value_obj, &vector_m1_obj, &vector_m2_obj, &matrix_obj))
    {
        return NULL;
    }
    ...
    if (PyIs_NativeInt(points_2_obj))
    {
        points_2 = PyInt64_to_Int64(points_2_obj);
    }
    else
    {
        PyErr_SetString(PyExc_TypeError, "Expected an argument of type int for argument points_2");
        return NULL;
    }
    if (PyIs_NativeFloat(domain_nb_obj))
    {
        domain_nb = PyDouble_to_Double(domain_nb_obj);
    }
    else
    {
        PyErr_SetString(PyExc_TypeError, "Expected an argument of type float for argument domain_nb");
        return NULL;
    }
    ...
    if (pyarray_check(ovlp_value_obj, NPY_DOUBLE, INT64_C(2), NPY_ARRAY_C_CONTIGUOUS))
    {
        ovlp_value = pyarray_to_ndarray(ovlp_value_obj);
    }
    else
    {
    ...
    bound_ovlp_value = nd_data(&ovlp_value);
    bound_ovlp_value_shape_1 = nd_ndim(&ovlp_value, INT64_C(0));
    bound_ovlp_value_shape_2 = nd_ndim(&ovlp_value, INT64_C(1));
    bound_ovlp_value_stride_1 = nd_nstep_C(&ovlp_value, INT64_C(0));
    bound_ovlp_value_stride_2 = nd_nstep_C(&ovlp_value, INT64_C(1));
...
static PyMethodDef mod_r4ni7ix97uiy_methods[] = {
    {
...
static struct PyModuleDef mod_r4ni7ix97uiy_module = {
    ...
    0,
    mod_r4ni7ix97uiy_methods,
};

/*........................................*/

PyMODINIT_FUNC PyInit_mod_r4ni7ix97uiy(void)
{
    PyObject* mod;
    static void* Pymod_r4ni7ix97uiy_API[0];
    PyObject* c_api_object_0001;
    mod = PyModule_Create(&mod_r4ni7ix97uiy_module);
    if (mod == NULL)
    {
        return NULL;
    }
    c_api_object_0001 = PyCapsule_New((void *)Pymod_r4ni7ix97uiy_API, "mod_r4ni7ix97uiy._C_API", NULL);
