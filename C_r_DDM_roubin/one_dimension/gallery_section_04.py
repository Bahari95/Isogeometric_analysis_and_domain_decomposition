...
#---1 : In uniform mesh
@types('int', 'int', 'int', 'int', 'int[:,:]', 'int[:,:]', 'double[:,:,:,:]', 'double[:,:,:,:]', 'double[:,:]', 'double[:,:]', 'double[:,:]', 'double[:,:]', 'real[:]', 'real[:]', 'double[:]', 'real', 'real', 'int', 'double[:]')
def assemble_vector_ex01(ne1, ne2, p1, p2, spans_1, spans_2, basis_1, basis_2, weights_1, weights_2, points_1, points_2, knots_1, knots_2, vector_d, ovlp_value, S_DDM, domain_nb, rhs):
    ...
    # ...
    lcoeffs_d[:] = vector_d[span_2 : span_2+p2+1]
    u   = 0.
    ux  = 0.
    ...
    for il_1 in range(0, p1+1):
        i1       =  span_1 - p1 + il_1
        rhs[p1+i1] += (neum_sign*ux+S_DDM*u)*basis1[0,il_1]
    # ...
