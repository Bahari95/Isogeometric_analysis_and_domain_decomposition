...
class DDM_Helmholtz(object):
    ...
    def solve(self,  x_d, x_dc):
       ...
       if self.domain_nb == 0 :
	       ...
	       x_dh[-1] = np.cos(self.Kappa*self.ovlp_value) # x_d
	       x_dh[0]  =  1.
	       u_d.from_array(self.V, x_dh)
	       x_dhc                  = zeros(self.V.nbasis)
	       x_dhc[-1] =  np.sin(self.Kappa*self.ovlp_value) #x_dc
	       u_dc.from_array(self.V, x_dhc)
       else :
	       x_dh                  = zeros(self.V.nbasis)
	       x_dh[0]  = np.cos(self.Kappa*self.ovlp_value)
	       u_d.from_array(self.V, x_dh)
	       x_dhc                  = zeros(self.V.nbasis)
	       x_dhc[0] = np.sin(self.Kappa*self.ovlp_value)
	       u_dc.from_array(self.V, x_dhc)
       ...
       #... Dirichlet nboundary
       if self.domain_nb == 0 :
	       xr                    = [1.] + list(x[:self.n_basis]) + [x_dh[-1]]
	       u.from_array(self.V, xr)
	       # ...       
	       xc                    = [0.] + list(x[self.n_basis:]) + [x_dhc[-1]]
	       v.from_array(self.V, xc)
       else :
	       xr                    = [x_dh[0]] + list(x[:self.n_basis])
	       u.from_array(self.V, xr)
	       # ...       
	       xc                    = [x_dhc[0]] + list(x[self.n_basis:])
	       v.from_array(self.V, xc)
